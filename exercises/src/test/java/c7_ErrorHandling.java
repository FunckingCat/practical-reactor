...
public class c7_ErrorHandling extends ErrorHandlingBase {
    ...
    public void houston_we_have_a_problem() {
        ...
        Flux<String> heartBeat = probeHeartBeatSignal().timeout(Duration.ofSeconds(3)).doOnError(
                e -> errorRef.set(e)
        )
    ...
    public void under_the_rug() {
        Flux<String> messages = messageNode().onErrorResume(i -> Mono.empty());
        //todo: change this line only
        ;
    ...
    public void good_old_polling() {
        //todo: change code as you need
        Flux<String> alerts = nodeAlerts().repeatWhenEmpty(
                it -> it.delayElements(Duration.ofSeconds(1))
        ).repeat();
